...
// Mapa operacional simplificado: vetorial, nítido e leve.
// A visão geral é esquemática; posições exatas de equipamentos permanecem nas pranchas técnicas e na validação de campo.
document.addEventListener('DOMContentLoaded',()=>{
  const img=document.getElementById('plant');
  if(!img) return;
  img.src='mapa-operacional.svg';
  img.dataset.quality='vector';
  img.style.width='1500px';
  img.style.maxWidth='none';
  img.style.imageRendering='auto';
  img.title='Mapa operacional simplificado — visão geral por setores';
  const box=img.closest('.mapbox')||img.parentElement;
  if(box){
    box.dataset.mapQuality='vector';
    box.setAttribute('aria-label','Mapa operacional simplificado e vetorial. Use zoom e arraste para navegar.');
  }
});